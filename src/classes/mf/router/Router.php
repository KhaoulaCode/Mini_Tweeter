...
class Router extends AbstractRouter {
    ...
    public function addRoute(string $name, string $action, string $ctrl, string $level = TweeterAuthentification::ACCESS_LEVEL_NONE): void{
    ...
    public function run() : void{
        if(isset($this->request->get['action'])){
            ...
            if ($route){
                if (TweeterAuthentification::checkAccessRight($route[1])){
                    $ctrl = new $route[0]();
                    $ctrl->execute();
                } else {
